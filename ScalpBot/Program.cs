...
//        var client = new ToobitClient(apiKey, secretKey);

//        // 🔹 داده عمومی
//        //var order = await client.PlaceOrderAsync("ADAUSDT", "BUY_OPEN", "LIMIT", 100);
//        var pnl = await client.GetTodayPnlAsync();
        ...
        while (true)
        {
            foreach (var symbol in topSymbols)
            {
                try
                {
                    ...
                    var psarList = Indicators.PSAR(highs, lows);
                    decimal lastPsar = psarList[^1];
                    decimal lastPrice = closes[^1];
                    bool psarTrendUp = lastPrice > lastPsar;

                    // write last60 CSV
                    var csvPath = Path.Combine("LSTM_Model", "data", $"{symbol}.csv");
                    ...
                    // write last60 CSV
                    var csv60Path = Path.Combine("LSTM_Model", "data", $"{symbol}_last60.csv");
                    using (var sw = new StreamWriter(csv60Path))
                    {
                        sw.WriteLine("open,high,low,close,volume");
                    ...
                    var modelPath = Path.Combine("LSTM_Model", "model", $"model_{symbol}.h5");
                    ...
                    var decision = analyzer.Decide(emaDir, rsi, obi, closes.Last(), vwap, macd - macdSignal, atr, probUp, bbUpper, bbLower, psarTrendUp);
                    ...
                    Console.WriteLine($"{symbol} | price={closes.Last():F6} | ProbUp={probUp:F2} => {decision}");
